...
def run_ppmi_lsa_pipeline(count_df, k):
    ...
    return full_word_similarity_evaluation(ppmi_lsa)
...
def run_small_glove_evals():
    ...
    glv = GloVe(max_iter=10)
    giga20_glv = glv.fit(giga20)
    glv_dict[10] = full_word_similarity_evaluation(pd.DataFrame(giga20_glv, index=giga20.index)).loc['Macro-average']
    print(glv_dict[10])
    glv.sess.close()

    glv = GloVe(max_iter=100)
    giga20_glv = glv.fit(giga20)
    glv_dict[100] = full_word_similarity_evaluation(pd.DataFrame(giga20_glv, index=giga20.index)).loc['Macro-average']
    print(glv_dict[100])
    glv.sess.close()
    
    glv = GloVe(max_iter=200)
    giga20_glv = glv.fit(giga20)
    glv_dict[200] = full_word_similarity_evaluation(pd.DataFrame(giga20_glv, index=giga20.index)).loc['Macro-average']
    print(glv_dict[200])
    glv.sess.close()
...
def ttest(df):
    ...
    outer = np.outer(row_total, column_total)
    denominator = outer**0.5
    numerator = df - outer
...
def subword_enrichment(df, n=4):
    ...
    char_level_rep = {}
    for word in words:
        char_level_rep[str(word)] = vsm.character_level_rep(str(word), df_ngrams, n)
    ...
    subword_rep = {}
    for word in words:
        word_rep = df.loc[str(word)]
        char_rep = char_level_rep[str(word)]
        subword_rep[word] = word_rep + char_rep
    ...
    df_array = []
    for word in words:
        df_array.append(subword_rep[str(word)])
        
    return pd.DataFrame(df_array, index=words)
    ##### YOUR CODE HERE
...
if 'IS_GRADESCOPE_ENV' not in os.environ:
    ...
    def system(vsmdata):
        x = vsmdata
#         x = subword_enrichment(x)
#         x = vsm.tfidf(x)
        x = vsm.pmi(x, positive=True)
        x = ttest(x)
        x = x.apply(vsm.length_norm, axis=1)
        x = TorchAutoencoder(max_iter=200, hidden_dim=1000, eta=0.001).fit(x)
        return x

#     data = pd.read_csv(os.path.join(VSM_HOME, 'imdb_window5-scaled.csv.gz'), index_col=0)
    data = pd.read_csv(os.path.join(VSM_HOME, 'giga_window5-scaled.csv.gz'), index_col=0)
#     data = pd.read_csv(os.path.join(DATA_HOME, 'glove6B300d-retrofit-wn.csv.gz'), index_col=0)
    data_new = system(data)
    result = full_word_similarity_evaluation(data_new)
    display(result)
